...
3. Run `.sre/env-up.sh <branch>` — builds dev stage, brings up services, waits for healthcheck, prints URL.

## On failure
- Step 3 non-zero exit: stop and escalate. Do not retry without escalation.
- Healthcheck timeout: run `DOCKER_HOST=$DEV_DOCKER_HOST docker compose -p $BRANCH_SLUG logs --tail=100 master` and escalate.
