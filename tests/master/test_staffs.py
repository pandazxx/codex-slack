...
def _make_topic(client, ws_id):
    return client.post(f"/api/workspaces/{ws_id}/topics", json={"subject": "work", "repo_ref": "master"}).json()
...
def test_mention_routes_to_workspace_staff(client):
    with patch("src.master.main.build_mqtt_client") as mock_build:
        ...
        with TestClient(app) as c:
            ...
            topic_id = c.post(f"/api/workspaces/{ws_id}/topics", json={"subject": "t", "repo_ref": "master"}).json()["id"]
...
def test_workspace_session_scope_shares_across_topics(client):
    with patch("src.master.main.build_mqtt_client") as mock_build:
        ...
        with TestClient(app) as c:
            ...
            t1 = c.post(f"/api/workspaces/{ws_id}/topics", json={"subject": "t1", "repo_ref": "master"}).json()["id"]
            t2 = c.post(f"/api/workspaces/{ws_id}/topics", json={"subject": "t2", "repo_ref": "master"}).json()["id"]
