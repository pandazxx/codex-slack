"""Tests for GET /hello-world endpoint."""
...
class TestHelloWorld:
    ...
    def test_hello_world_returns_200(self, _base_env) -> None:
        """GET /hello-world must return HTTP 200."""
        with TestClient(app) as client:
            response = client.get("/hello-world")
        assert response.status_code == 200

    def test_hello_world_returns_message(self, _base_env) -> None:
        """GET /hello-world must return a message in the response."""
        with TestClient(app) as client:
            response = client.get("/hello-world")
        assert response.status_code == 200
        body = response.json()
