...
## 2026-05-18 — Agent self-destruct via `_kill_proc_tree` + `MagicMock().__int__() == 1` in under-mocked tests

*Summary:* The recurring SIGKILL of `codex-agent-e6d8e527-…` is the agent killing itself. The user uses workspace `e6d8e527` for AI-assisted development on the codex-slack project. When the LLM (claude-code/codex CLI) is prompted to run the agent's own test suite (`python -m pytest tests/agent/test_mqtt_loop.py`), pytest executes `test_run_claude_timeout` or `test_run_codex_turn_failed_is_error`. Both tests create a bare `proc = MagicMock()` and patch `subprocess.Popen` to return it, but **do not** patch `os.killpg`. When `_run_claude`/`_run_codex` hits the timeout/error branch, it calls `_kill_proc_tree(proc)` → `os.killpg(proc.pid, SIGKILL)`. `proc.pid` is a MagicMock; the C extension converts it to int via `MagicMock().__int__()`, which **returns 1 by default**. So the syscall becomes `killpg(1, SIGKILL)` → `kill(-1, SIGKILL)`, which broadcasts SIGKILL to every process the caller can signal in its PID namespace. Inside the container that's the entire agent process tree (agent main, claude CLI, bash, pytest itself). tini (PID 1) sees its child gone and exits with `exitCode=137`. Docker's `unless-stopped` policy bounces the container; rc6's `_active_procs` replay surfaces it as `agent-killed`.

*Root cause discovery:* Auditd (installed 2026-05-18 ~01:50 UTC as `key="sigkill"`) captured the syscall for the 07:00:11 UTC incident. PROCTITLE decoded to `python -m pytest tests/agent/test_mqtt_loop.py -x --tb=short`, `comm="python" exe="/usr/local/bin/python3.11" subj=docker-default a0=ffffffff a1=9`. `a0=ffffffff` (= -1 in pid_t) corresponds to `kill(-1, 9)`, which is `killpg(1, 9)` per glibc. The agent codebase has exactly one `os.killpg` call site (`src/agent/mqtt_loop.py:65`), and reproducing locally confirmed `int(MagicMock())` returns 1.

*Why every earlier hypothesis missed it:* Master code was searched exhaustively for kill paths (`pause_agent`, `stop_agent`, `remove(force=True)`) — all ruled out by dockerd-journal semantics (would have shown `manualRestart=true` or container-ID change). Host-side killers were ruled out (no auditd entries from host context, no oomd, no security agents, no host-level cgroup pressure). OOM was ruled out for every kill. The "master restart fixed it" observation was coincidence: the user simply hadn't asked the LLM to run the tests for ~4 hours after the restart; the bug recurred the moment the LLM ran pytest again at 07:00 UTC. The smoking gun only became visible when auditd captured the syscall record with PROCTITLE.

*Fix applied (v4.15-rc9, this commit):*
- **Defensive guard** in `_kill_proc_tree` (`src/agent/mqtt_loop.py:58`): refuses any `pgid <= 1` and logs `agent.kill_proc_tree_refused_unsafe_pgid pgid=N`. Legitimate subprocess pgids in a tini container are always >= 2; pgid=0 (caller's group) or pgid=1 (init/tini) can only be self-destruct. This guard would have prevented the entire incident class — including the original prod incident, the cluster of 6 kills, and the recurrence at 07:00.
- **Test fixes** in `tests/agent/test_mqtt_loop.py`: `test_run_claude_timeout`, `test_run_codex_turn_failed_is_error`, and `test_publish_interrupted_all` now (a) set `proc.pid = 999999` explicitly (safe sentinel that passes the guard) and (b) patch `src.agent.mqtt_loop.os.killpg` (defense in depth — if the safe pid is missed, no real syscall fires).
- **New tests** validating the guard: `test_kill_proc_tree_refuses_pgid_1`, `test_kill_proc_tree_refuses_pgid_0`, `test_kill_proc_tree_refuses_magicmock_pid`, `test_kill_proc_tree_kills_legitimate_pgid`.

*Prevention:* The `MagicMock().__int__() == 1` gotcha is general — any code that integer-converts a mock attribute can hit it. Whenever a test mocks a subprocess-like object and the production code-under-test passes `proc.pid` (or any int-typed mock attr) into a syscall, either set the attr explicitly or patch the syscall. The guard in `_kill_proc_tree` is the structural fix; the test patterns above are the disciplined approach for future tests. *Side note:* anyone who has ever run these tests locally has issued `kill(-1, 9)` on their machine — on Linux this kills processes owned by the user; on macOS the behaviour is the same. The bug was always there; it only became visible when prod's LLM started running the tests inside a container where the blast radius was the whole container.
...
- ~~Should agent containers carry an explicit `mem_limit` so future OOMs are *visible*?~~ Done in v4.15-rc8 (see fix below).
...
*Host-side investigation summary (2026-05-18 evening):* Both incident windows were swept across full systemd journal, containerd log, audit log (auditd not installed at the time), systemd-oomd (not installed), cron+at+systemd-timers, SSH/auth log, security/monitoring agents (none installed), dmesg, and cgroup scope events. **Findings:** (1) only the `e6d8e527` agent container restarted in either kill minute — no other agent or service was affected, ruling out host-wide killers; (2) the only scheduled job near either window is the hourly `docker system prune -af` cron, which doesn't touch running containers; (3) SSH login from `10.10.10.222` at 2026-05-17 08:48:51 was the operator (panda's laptop) reactively checking after the kill, not causative; (4) systemd scope deactivation (`docker-<id>.scope: Deactivated successfully.`) is *responsive*, triggered by containerd's `task-delete` event after the in-container process already died — not a sign of systemd killing the container. The kill chain we see is: in-container PID 1 (or process group) exits with signal 9 → containerd shim disconnects → dockerd applies restart policy → systemd cleans up the cgroup scope. Origin of the SIGKILL is *inside* the container's PID namespace and invisible to dockerd journal alone.

*Auditd installed on prod (2026-05-18 ~01:50 UTC) as diagnostic instrumentation:* Package `auditd` installed via apt (Ubuntu 24.04). Persistent rule file at `/etc/audit/rules.d/50-sigkill.rules` captures every `kill(*, SIGKILL)`, `tkill(*, SIGKILL)`, `tgkill(*, *, SIGKILL)` syscall on the host with tag `key="sigkill"`. Smoke-tested with `bash` builtin kill and `/usr/bin/kill`; both captured with `comm`, `exe`, `pid`, `auid`, target PID. Reliable lookup is `grep 'key="sigkill"' /var/log/audit/audit.log` (the canonical `ausearch -k sigkill` returns no matches on this host, possibly a `rules.d` indexing quirk). Retention is 8 MB × 5 files = 40 MB, ample for the low-volume rule. Runbook §2 Step 8 documents the post-incident query path. *Caveat:* the rule traces `kill` / `tkill` / `tgkill` syscalls only — it does **not** catch process termination via `cgroup.kill` writes (Linux 5.14+ cgroup v2 kill-the-cgroup mechanism), eBPF kill helpers, or kernel-internal kills outside the audited syscall path. Step 9 of the runbook covers those.

*Open question about workspace specificity:* Both incidents targeted the same workspace, `e6d8e527`. Worth probing once the next kill happens with audit instrumentation in place — if `comm=python` or `comm=tini` show up as targets (target = `a0`) but no `comm=<killer>` syscall pair exists, the kill is bypassing the audited syscall and we need Step 9 hypotheses (cgroup.kill, eBPF, kernel). If a real killer comm shows up, the investigation collapses immediately.

*Agent container memory limit added (v4.15-rc8):* `src/master/agent_runner.spawn_agent` now accepts a `mem_limit` parameter; `MasterSettings.agent_mem_limit` reads `MASTER_AGENT_MEM_LIMIT` env (default `"1g"`; empty string disables). All three call sites (`main.py:respawn`, `workspaces.py:create_workspace`, `workspaces.py:restart_agent`) pass it. Three reasons: (1) makes future OOM kills *visible* — `docker inspect OOMKilled=true` will be set when the cgroup limit is hit, where previously an unlimited container could blow up host memory without leaving the diagnostic fingerprint; (2) prevents one runaway agent from starving master/mosquitto on a small host (prod has 3.8 GiB total); (3) the 1g default fits ~2 agents alongside master (1g) and mosquitto (128m) on the current prod host class, and idle agents currently use only 22–46 MiB so the limit has plenty of headroom. *Deploy note:* default takes effect on next agent respawn after master picks up the new image. To raise (e.g., for heavy git/LLM workloads), set `MASTER_AGENT_MEM_LIMIT=2g` in the master container's env. To disable (revert to no limit), set `MASTER_AGENT_MEM_LIMIT=""`.

*2026-05-18 evening kill cluster — master restart was the cutoff, root cause not identified:* User deployed rc8 master at `02:22:23 UTC`. Before that point, the e6d8e527 agent container had been SIGKILL'd **six times in 2 h 26 min** — at 00:56:01, 01:11:18, 01:29:30, 01:30:40, 01:32:25, 02:22:07. All six matched the same dockerd pattern: same container ID (`0f5610519130…`), `exitCode=137`, `manualRestart=false`, `restartCount++`, no OOM (kernel and dockerd journals clean for every window, `OOMKilled=false`). After rc8 master came up at 02:22:23, **zero kills**. The agent container was **not** upgraded — `respawn_skip ... reason=already_running` was logged by rc8 master at 02:22:25, so the existing agent kept running on the rc6 image without `mem_limit`. That rules out `mem_limit` (rc8's only behavioural code change) as the cause: it never applied to this agent. The fix is therefore the master process restart (in-memory state reset), not the code change.

*Searches that ruled out master code paths:* `pause_agent` (uses `container.stop()` → dockerd records as deliberate stop → policy does **not** restart for `unless-stopped`) — doesn't match. `stop_agent` and `spawn_agent`'s pre-cleanup (use `containers.get(name).remove(force=True)` → destroys container → next start would be a NEW container ID) — doesn't match (all 6 kills had the same container ID). `start_agent_if_stopped` only acts on already-stopped containers. `refresh_auth` does `container.exec_run(["gh","auth","setup-git"])`, not a kill. No call to `container.kill`, `client.api.kill`, `client.api.signal` exists anywhere in `src/master/`. So master code cannot have directly killed the agent in a way that matches the observed dockerd-journal pattern. The kill came from **inside the container's PID namespace**, but old master logs were purged when rc8 replaced its container (default `json-file` log driver), so we can't see what master was doing in the 60 s before each kill.

*Working hypothesis:* Old master's in-memory state (most plausibly its MQTT client state — subscriptions, pending pings, in-flight prompts) was corrupted, causing it to send the agent something that triggered an in-container fatality — either a fatal codex CLI command, a self-kill via Python subprocess management race, or a flood of duplicate prompts/cancels driving the agent into a crash. The agent log around the kill windows shows repeated `git worktree add` failures (exit 128) which would not normally be fatal but could be a symptom of a tight-loop state. The cluster pattern (kills spacing tightened from ~25 min to ~1 min) is consistent with master state degrading over time. Master restart cleared the state and the symptom stopped. **Not proven** — only the next incident, with auditd now running, will tell us which process inside the container is being killed and by whom.

*Decision:* Wait for the next incident rather than try to reproduce or add speculative defensive logging. The auditd `key="sigkill"` lookup (runbook §2 Step 8), the rc7 `agent.signal_received` log line, and `docker inspect OOMKilled` will together pin the root cause once we have a fresh data point. Open follow-up if recurrence is slow or never: switch dockerd log driver to journald so the next master replacement doesn't lose evidence again.
